...
def drive_straight(speed=50, pos=0, distance_threshold=1000, wall_side=0, pid_ks=(1, 0, 0)):
    ...
    try:
        while True:
            ...
            #     dc_speed(0)
            #     break
            if current_ms - last_turn > 2000:
                # print(left_distance, right_distance)
                if left_distance > 700 or right_distance > 700:
                    if turns == 12:
                        dc_speed(-50)
                        finish_position = encoder.data[1] - start_distance
                        print("GOING BACK")
                        while encoder.data[1] >= finish_position:
                            pass
                        dc_speed(0)
                        break
                    if turns == 0:
